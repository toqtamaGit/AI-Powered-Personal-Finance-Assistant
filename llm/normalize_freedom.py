...
_KNOWN_CITIES = {
    'ASTANA', 'NUR-SULTAN', 'ALMATY', 'SHYMKENT', 'KARAGANDA', 'TARAZ',
    'PAVLODAR', 'SEMEY', 'KOSTANAY', 'UST-KAMENOGORSK', 'PETROPAVLOVSK',
    'ATYRAU', 'ORAL', 'URALSK', 'KYZYLORDA', 'TALDYKORGAN', 'ARKALYK',
    'AKTOBE', 'EKIBASTUZ', 'RUDNY', 'ZHEZKAZGAN', 'TURKESTAN',
    'ARYS', 'AKKOL',
}
...
_CITY_RE = re.compile(
    r'\s+(' + '|'.join(re.escape(c) for c in sorted(_KNOWN_CITIES, key=len, reverse=True)) + r')(?:\s+Q\.)?$',
    flags=re.IGNORECASE,
)
...
def parse_freedom_details(
    ...
    # 5. Clean up quotes/brackets and normalize whitespace
    text = re.sub(r'["\'\[\]\(\)]', '', text)
...
def normalize_csv(input_file: str, output_file: str) -> pd.DataFrame:
    ...
    # Parse each row
    parsed = df.apply(
        lambda row: parse_freedom_details(
            row.get('details', ''),
            row.get('operation', ''),
        ),
        axis=1,
    ...
    df['country'] = [x[4] for x in parsed]
